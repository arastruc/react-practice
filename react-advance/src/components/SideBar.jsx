...
const SideBar = ({ projectState, selectProject, addProject }) => {
    ...
    <aside className="bg-stone-900 md:w-1/3 px-8 py-16 text-stone-50 rounded-r-lg w-72">
      ...
      <ul className="m-8 text-lg">
        {projects.map(({ title }, index) => (
          <li
            key={index}
            className={`py-2 px-4 my-4 text-lg text-stone-300  hover:bg-stone-500 hover:text-stone-100 ${
              title === selectedProject?.title ? "bg-stone-500" : "bg-stone-700"
            }`}
          >
            <button
